...
namespace Partition.Managing
{
    public class LocalCommandRunner : MonoBehaviour
    {
        ...
        {
            _partitionRunner.CreateFuzzyPartitionWithFixedCenters(GetSettings(), _drawMistrustRate, _mistrustRate);
        }

        [Button("Create fuzzy partition with centers placing")]
        public void CreateFuzzyPartitionWithCentersPlacing()
        {
            _partitionRunner.CreateFuzzyPartitionWithCentersPlacing(GetSettings(), _drawMistrustRate, _mistrustRate);
        }
